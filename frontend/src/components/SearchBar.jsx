...
export default function SearchBar({ fetchFromSearch, setSearchResults }) {
    ...
    useEffect(() => {
        if (query.length > 2) return doSearch(query);
    ...
    */
    ...
    return <nav aria-label="Search Bar Navigation" className='search-navigation flex flex-row'>
        <form id="search-form" className='single' onSubmit={ (event) => event.stopPropagation() }>
            ...
            {input?.current?.value ? input.current.value.length > 0 ? <button className="remove-search-input w-unset" onClick={clearBar}>x</button> : null : null}
        </form>
    </nav>;
}