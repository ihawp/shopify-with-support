...
export default function Header() {
    ...
    return <header id="header">
            ...
            <nav id="cart-navigation" aria-label="Cart Navigation" className="flex flex-row justify-end items-center w-50-sm-unset gap-1">
                <NavLink to="/cart" className="flex flex-row items-center gap-0-5 cart" title={`Cart: ${cartItems.length} Items`}>
                    <button className="cart-button py-1 px-0-5 flex flex-row items-center gap-1" aria-label="View Your Cart">
            ...
            <div className='logo header'>
                <NavLink to={'/'} aria-label='Go to Shop/Home Page' className="display-sm">
                    <Logo />
                </NavLink>
            </div>
        </header>;
