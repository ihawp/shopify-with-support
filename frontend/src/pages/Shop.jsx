...
export default function Shop() {
  ...
  const fetchFromSearch = async (val) => {
    ...
    const searchQuery = `
      query SearchProducts($query: String!, $first: Int, $types: [SearchType!]) {
  ...
  const fetchProducts = async (cursor = null) => {
    ...
    try {
      ...
      const response = await client.request(query, productVariables);
  ...
  return <main className='flex flex-col items-center gap-1'>
      ...
      <section className='flex flex-row gap-1 justify-between'>
        ...
        <SearchBar fetchFromSearch={fetchFromSearch} setSearchResults={setSearchResults} />
      </section>

      <section id="shop" className='shop-products flex flex-row flex-wrap'>
        {(searchResults !== null ? searchResults : products).map(({ node }, index) => {
          return <Product node={node} key={node.id} />
        })}
      </section>
